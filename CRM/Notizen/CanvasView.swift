...
//  CRM
//
//  Created by Noel Jander on 23.07.18.
...
class CanvasView: UIImageView {
    ...
    private var drawColor: UIColor = UIColor.yellow
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        // Speichert das neue Bild auf in dem Stack
        let annotation = Annotation(color: drawColor, image: self.drawingImage!)
        store.save(annotation: annotation)
    }
    
    override func touchesCancelled(_ touches: Set<UITouch>, with event: UIEvent?) {
        image = self.drawingImage
        // Speichert das neue Bild auf dem Stack
        let annotation = Annotation(color: drawColor, image: self.drawingImage!)
        store.save(annotation: annotation)
    }
